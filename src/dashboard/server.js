...
import { calculateRSI, evaluateMarketRegime } from '../services/indicatorService.js';
import {
  runBacktest,
...
import { generateLoginUrl, generateSession, getKite, isKiteAvailable, validateSession } from '../services/kiteService.js';
...
app.get('/api/signals', async (_req, res) => {
  try {
    ...
    if (config.kiteEnabled && isKiteAvailable() && isMarketHours()) {
      ...
      } catch { /* Nifty live quote is best-effort */ }
    }
    ...
    for (const symbol of watchlist) {
      ...
      signals.push({
        ...
        positionSize: calculateWeightedPositionSize(data.price, signal.supportLevel, signal),
        stopLoss: Math.round(data.price * (1 - config.stopLossPercent) * 100) / 100,
  ...
  try {
    ...
    for (const symbol of watchlist) {
      if (quotes[symbol]) {
        prices[symbol] = {
          ...quotes[symbol],
          source: quotes[symbol].source || (quotes[symbol].timestamp ? 'kite' : 'yahoo_daily'),
        };
      }
    }
    res.json({ prices, kiteAvailable: isKiteAvailable(), marketOpen: isMarketHours() });
  } catch (err) {
...
app.get('/api/kite/status', async (_req, res) => {
  if (!config.kiteEnabled) {
    return res.json({ enabled: false, configured: false, sessionValid: false, available: false });
  }
...
app.get('/api/kite/login', async (_req, res) => {
  if (!config.kiteEnabled) {
    return res.status(400).json({ error: 'Kite is not enabled (set KITE_ENABLED=true)' });
  }
...
server.on('listening', async () => {
  ...
  if (config.kiteEnabled) {
    ...
    logger.info(`Kite session on dashboard startup: ${valid ? 'VALID' : 'INVALID'}`);
  }
});
